...
namespace EnterpriseLibrary.SemanticLogging.Tests.Formatters
{
    [TestClass]
    ...
    public abstract class given_json_event_text_formatter : ContextBase
    {
        ...
        public class when_writing_to_null_writer : given_json_event_text_formatter
        {
            ...
            protected override void When()
            {
                try
                {
                    formatter.WriteEvent(new EventEntry(Guid.NewGuid(), 0, string.Empty, new System.Collections.ObjectModel.ReadOnlyCollection<object>(new object[0]), DateTimeOffset.MaxValue, new EnterpriseLibrary.SemanticLogging.Schema.EventSourceSchemaReader().GetSchema(logger).Values.First()), null);
                    Assert.Fail("should have thrown");
                }
                catch (ArgumentNullException e)
                {
